...
object KafkaDirectWordCountV2 {
  def main(args: Array[String]): Unit = {
    ...
    val topic = "test1"
    //broker地址
    val brokerList = "myspark:9092"

    //zk地址以后把新的偏移量写入zk

    val zkPath = "myspark:2181,myspark:2182,myspark:2183"
    //创建 stream 时使用的 topic 名字集合，SparkStreaming可同时消费多个topic
    val topics: Set[String] = Set(topic)
    //创建一个 ZKGroupTopicDirs 对象,其实是指定往zk中写入数据的目录，用于保存偏移量
    val topicDirs = new ZKGroupTopicDirs(group, topic)
    //获取 zookeeper 中的路径 "/g001/offsets/wordcount/"
    val zkTopicPath: String = topicDirs.consumerOffsetDir//s"${topicDirs.consumerOffsetDir}"
    //准备kafka的参数
    val kafkaParams = Map(
    ...
    val zkClient: ZkClient = new ZkClient(zkPath)
    ...
    if (children > 0) {
      ...
      //这个会将 kafka 的消息进行 transform，最终 kafak 的数据都会变成 (kafka的key, message) 这样的tuple
